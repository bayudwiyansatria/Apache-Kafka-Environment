...
if [ $(id -u) -eq 0 ]; then
    ...
    echo -e 'export KAFKA_CONF_DIR=${KAFKA_HOME}/etc/hadoop' >> $profile;
        ...
        if [ -n "$workeraccept" ] ; then 
            if [ "$workeraccept" == "y" ] ; then
                while [ "$workeraccept" == "y" ] ; do 
                    ...
                    ssh $worker "cp $KAFKA_HOME/config/server.properties $KAFKA_HOME/config/server-$broker.properties";
